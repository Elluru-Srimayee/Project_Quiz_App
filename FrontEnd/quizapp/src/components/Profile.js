...
function Profile() {
  ...
  const fetchUserResults = async () => {
    try {
      ...
      const quizIdsResponse = await axios.get(`http://localhost:5057/api/QuizResult/answered-quiz-ids/${username}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
      ...
      for (const quizId of answeredQuizIds) {
        try {
          const totalResultResponse = await axios.get(`http://localhost:5057/api/QuizResult/totalscore/${quizId}/${username}`, {
            headers: {
              Authorization: `Bearer ${token}`,
            },
          });
